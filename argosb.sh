...
up(){
rm -rf /usr/bin/agsb
curl -L -o /usr/bin/agsb -# --retry 2 --insecure https://raw.githubusercontent.com/gh1852/FList/refs/heads/master/argosb.sh
...
sed -i '/yonggekkk/d' ~/.bashrc
echo "export nix=y uuid=${uuid} vmpt=${port_vm_ws} agn=${ARGO_DOMAIN} agk=${ARGO_AUTH} && bash <(curl -Ls https://raw.githubusercontent.com/gh1852/FList/refs/heads/master/argosb.sh)" >> ~/.bashrc
source ~/.bashrc
fi
sleep 2
...
argocore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/cloudflare/cloudflared | grep -Eo '"[0-9.]+",' | sed -n 1p | tr -d '",')
echo "下载cloudflared-argo最新正式版内核：$argocore"
curl -L -o nixag/cloudflared -# --retry 2 https://github.com/cloudflare/cloudflared/releases/download/2025.4.2/cloudflared-linux-$cpu
